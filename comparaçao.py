...
def encontrar_imagem(filename):
    for c in range(1, 7):
        folder = os.path.join(img_dir, str(c))
        for ext in exts:
...
for xml_file in os.listdir(anno_dir):
    ...
    img_path = encontrar_imagem(filename)
    if img_path is None:
        print(f"⚠️ Imagem não encontrada em nenhuma classe: {filename}")
        continue
    ...
    for i, obj in enumerate(root.findall("object")):
        ...
        save_name = f"{filename}_seg_comparison.bmp"
