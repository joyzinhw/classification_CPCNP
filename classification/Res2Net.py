...
class SEModule(nn.Module):
    """
    Módulo Squeeze-and-Excitation para focar nos canais mais importantes.
    """
...
class Res2NetBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, downsample=None, stride=1, scales=4, groups=1, se=True, norm_layer=nn.BatchNorm2d):
        ...
        # 1x1 conv
        self.conv1 = nn.Conv2d(inplanes, bottleneck_planes, kernel_size=1, stride=stride, bias=False)
        self.bn1 = norm_layer(bottleneck_planes)

        # 3x3 convs (hierárquicas do Res2Net)
        ...
        # 1x1 conv final
        self.conv3 = nn.Conv2d(bottleneck_planes, planes * self.expansion, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, x):
        ...
        # Divisão em escalas (hierarquia Res2Net)
        ...
        for s in range(self.scales):
            if s == 0:
                ...
                # Conexão residual interna entre escalas
...
class Res2Net(nn.Module):
    def __init__(self, layers, num_classes, width=16, scales=4, groups=1,
                 zero_init_residual=True, se=True, norm_layer=nn.BatchNorm2d):
        super(Res2Net, self).__init__()
        
        # Configuração automática dos canais baseada no width
        planes = [int(width * scales * 2 ** i) for i in range(4)]
        self.inplanes = planes[0]

        # Entrada para 1 canal (Grayscale)
        ...
        # --- MELHORIA: DROPOUT ---
        # Crucial para evitar o overfitting que vimos nos seus resultados anteriores
        self.dropout = nn.Dropout(p=0.5)
        
        self.fc = nn.Linear(planes[3] * Res2NetBottleneck.expansion, num_classes)

        # Inicialização de pesos (Kaiming para Conv e constante para BN)
        ...
        # Inicialização zero para os ganhos residuais (ajuda na estabilidade inicial)
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, Res2NetBottleneck):
                    nn.init.constant_(m.bn3.weight, 0)

    def _make_layer(self, block, planes, blocks, stride=1, scales=4, groups=1, se=True, norm_layer=nn.BatchNorm2d):
        downsample = None
    ...
    def forward(self, x):
        ...
        # Aplicando Dropout antes da última camada Linear
        x = self.dropout(x)
